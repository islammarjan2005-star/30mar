...
unemployment_age_stats_card_ui <- function(id) {
  ...
  mod_govuk_data_vis_card_ui(
    id = ns("unemp_age_trend_card"),
    title = "Unemployment by Age Group",
    help_text = "Unemployment levels and rates by age group, seasonally adjusted.",
    ...
    table_content  = reactableOutput(ns("unemp_age_table"), height = "350px"),
    visual_content = plotlyOutput(ns("unemp_age_plot"),     height = "350px"),
    query = ns("sql_query"),
...
unemployment_age_stats_card_server <- function(id, conn = APP_DB$pool) {
  moduleServer(id, function(input, output, session) {
    ...
    codes <- reactive({
      ...
      if (sel == "Level (000s)") UNEMP_AGE_LEVEL_CODES else UNEMP_AGE_RATE_CODES
    })
    ...
    output$unemp_age_table <- reactable::renderReactable({
    ...
    output$unemp_age_plot <- plotly::renderPlotly({
      out <- dat(); req(nrow(out$data) > 0)
      dbt_ts_plot(
        df = out$data,
...
get_unemp_age_tbl <- function(codes) {
  ...
  base %>%
    dplyr::filter(.data$dataset_identifier_code %in% !!codes) %>%
    dplyr::mutate(time_period = !!period_sql) %>%
    dplyr::mutate(
      age_group = dplyr::case_when(
        .data$dataset_identifier_code %in% c("YBVH", "YBVK") ~ "16-17",
        .data$dataset_identifier_code %in% c("YBVN", "YBVQ") ~ "18-24",
        .data$dataset_identifier_code %in% c("YCGM", "YCGP") ~ "25-34",
        .data$dataset_identifier_code %in% c("YCGS", "YCGV") ~ "35-49",
        .data$dataset_identifier_code %in% c("LF28",  "LF2E") ~ "50-64",
        .data$dataset_identifier_code %in% c("K5HU",  "K5HW") ~ "65+",
        TRUE ~ NA_character_
      )
    ) %>%
    dplyr::select(
      time_period,
      dataset_id = .data$dataset_identifier_code,
      age_group,
      value
    )
}
